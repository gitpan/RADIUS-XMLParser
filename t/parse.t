...
dir_only_contains_ok($test_output_dir, [qw(tmp radius.xml)], "Assert that radius.xml has been created");
...
for my $key (keys %expect){
	ok($actual{$key} == $expect{$key},  "Assert that returned $key is equal to $expect{$key}")
}
